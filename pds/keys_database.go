package pds

import (
	"context"
	"fmt"
	"iter"
	"net/url"
	"sort"
	"strings"

	"github.com/aaronland/go-roster"
)

type ListKeysOptions struct {
	DID string
}

type KeysDatabase interface {
	GetKey(context.Context, string, string) (*Key, error)
	AddKey(context.Context, *Key) error
	DeleteKey(context.Context, *Key) error
	ListKeys(context.Context, *ListKeysOptions) iter.Seq2[*Key, error]
	Close() error
}

var keypairs_database_roster roster.Roster

// KeysDatabaseInitializationFunc is a function defined by individual keypairs_database package and used to create
// an instance of that keypairs_database
type KeysDatabaseInitializationFunc func(ctx context.Context, uri string) (KeysDatabase, error)

// RegisterKeysDatabase registers 'scheme' as a key pointing to 'init_func' in an internal lookup table
// used to create new `KeysDatabase` instances by the `NewKeysDatabase` method.
func RegisterKeysDatabase(ctx context.Context, scheme string, init_func KeysDatabaseInitializationFunc) error {

	err := ensureKeysDatabaseRoster()

	if err != nil {
		return err
	}

	return keypairs_database_roster.Register(ctx, scheme, init_func)
}

func ensureKeysDatabaseRoster() error {

	if keypairs_database_roster == nil {

		r, err := roster.NewDefaultRoster()

		if err != nil {
			return err
		}

		keypairs_database_roster = r
	}

	return nil
}

// NewKeysDatabase returns a new `KeysDatabase` instance configured by 'uri'. The value of 'uri' is parsed
// as a `url.URL` and its scheme is used as the key for a corresponding `KeysDatabaseInitializationFunc`
// function used to instantiate the new `KeysDatabase`. It is assumed that the scheme (and initialization
// function) have been registered by the `RegisterKeysDatabase` method.
func NewKeysDatabase(ctx context.Context, uri string) (KeysDatabase, error) {

	u, err := url.Parse(uri)

	if err != nil {
		return nil, err
	}

	scheme := u.Scheme

	i, err := keypairs_database_roster.Driver(ctx, scheme)

	if err != nil {
		return nil, err
	}

	init_func := i.(KeysDatabaseInitializationFunc)
	return init_func(ctx, uri)
}

// Schemes returns the list of schemes that have been registered.
func KeysDatabaseSchemes() []string {

	ctx := context.Background()
	schemes := []string{}

	err := ensureKeysDatabaseRoster()

	if err != nil {
		return schemes
	}

	for _, dr := range keypairs_database_roster.Drivers(ctx) {
		scheme := fmt.Sprintf("%s://", strings.ToLower(dr))
		schemes = append(schemes, scheme)
	}

	sort.Strings(schemes)
	return schemes
}
